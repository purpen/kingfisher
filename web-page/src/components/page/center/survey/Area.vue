<template>
  <div class="container min-height350">
    ...
    <v-menu currentName="area" currentTitle="地域分布"></v-menu>
    <div class="">
      ...
      <div class="item">
        <div class="title">
          <h3>地域分布</h3>
          <Date-picker type="daterange" confirm class="select-date" :options="areaDateOptions" :value="lastMonthDate()" placement="bottom-end" placeholder="选择日期" @on-change="changeDate" @on-ok="sureDate(1)"></Date-picker>
        </div>
        <div id="areaMap"></div>
      </div>   
    </div>
...
export default {
  ...
  methods: {
    ...
    sureDate (evt) {
      if (this.areaDate) {
        this.loadData(this.areaDate[0], this.areaDate[1])
      }
    },
    changeDate (date) {
      this.areaDate = date
  ...
  mounted: function () {
    ...
    self.areaChart = echarts.init(document.getElementById('areaMap'), 'macarons')
