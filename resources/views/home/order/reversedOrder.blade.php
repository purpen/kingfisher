            ...
            <table class="table table-bordered table-striped">
                ...
                @foreach($order_list as $order)
                    <tr>
                        ...
                        <td tdr="nochect">
                            <button class="btn btn-gray btn-sm mr-2r reversed_order" type="button" value="{{$order->id}}">反审</button>
...
@section('customize_js')
    ...
    $(".reversed_order").click(function () {
    var order_id = $(this).attr('value');
    var obj = $(this).parent().parent();
    $.post('{{url('/order/ajaxReversedOrder')}}',{'_token': _token,'order': [order_id]}, function (e) {
        if(e.status){
            obj.remove();
        }else{
            alert(e.message);
