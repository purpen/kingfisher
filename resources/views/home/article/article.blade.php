        ...
        <div class="container mainwrap">
            <div class="row">
                <div class="col-md-1">
                    {{--分页数量选择--}}
                    @if(!empty($product_id))
                    <form id="per_page_from" action="{{url('/saas/article')}}?id={{$product_id}}" method="POST">
                    @else
                    <form id="per_page_from" action="{{ url('/saas/article') }}" method="POST">
                        @endif
                   ...
                   <table class="table table-bordered table-striped">
                        ...
                        @foreach($articles as $article)
                            <tr>
                                ...
                                <td>
                                    @if ($article->status == 1)
                                        <a href="/saas/article/{{ $article->id}}/unStatus" class="btn btn-sm btn-danger  mr-2r">草稿箱</a>
                                    @else
                                        <a href="/saas/article/{{ $article->id}}/status" class="btn btn-sm btn-success  mr-2r">已审核</a>
                                    @endif
                                    <a class="btn btn-default btn-sm mr-2r" href="{{ url('/saas/article/edit') }}/{{$article->id}}">编辑</a>
                                    <a class="btn btn-default btn-sm mr-2r" href="{{ url('/saas/article/delete') }}/{{$article->id}}">删除</a>
